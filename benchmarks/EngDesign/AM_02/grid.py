...
class Grid:
    def __init__(self, robot_trajectory1, robot_trajectory2, grid_size = 30, time_steps = 20, pedestrian_size = 2):
        ...
        self.pedestrian_paths = {
            ...
            "robot1": robot_trajectory1,
            "robot2": robot_trajectory2
        }
        self.fig, self.ax = plt.subplots(figsize=(6, 6))
        self.trails = {key: [] for key in self.pedestrian_paths}

    def check_obstacle_collision(self, robot_name):
        # Check if the pedestrian collides with any static boxes
        num_collisions = 0
        for t in self.pedestrian_paths[robot_name]:
            _, x, y = t
    ...
    def check_pedestrian_collision(self, robot_name):
        size = self.pedestrian_size
        num_collisions = 0
        for tuple in self.pedestrian_paths[robot_name]:
            t, x1, y1 = tuple
            for pedestrian_id, path in self.pedestrian_paths.items():
                if pedestrian_id == robot_name:
                    continue
    ...
    def over_speed_limit(self, robot_name, speed_limit = 2):
        # Check if the robot exceeds the speed limit
        for i in range(1, len(self.pedestrian_paths[robot_name])):
            t1, x1, y1 = self.pedestrian_paths[robot_name][i-1]
            t2, x2, y2 = self.pedestrian_paths[robot_name][i]
...
if __name__ == "__main__":
    ...
    for i in range(1,3):
        ...
        print("Number of collisions with static obstacles:", num_obs_collisions)
        print("Number of collisions with walking pedestrians:", num_ped_collisions)
        print("Over speed limit:", speed_limit_exceeded)
    grid.save_animation()