"""Reference implementation for the SnAr multi-objective task."""
...
def _run_weighted_sobo(seed: int, budget: int, weight: float) -> list[dict]:
    ...
    expression = f"{weight:.6f}*(sty/13000)+(1-{weight:.6f})*(1-e_factor/500)"
    transform = MultitoSingleObjective(experiment.domain, expression=expression, maximize=True)
